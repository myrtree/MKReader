...
/* UTF16 string utilities */
static __inline__ int UTF16_strlen(const Uint16 *text)
{
	int size = 0;
	while ( *text++ ) {
		++size;
...
static __inline__ void UTF16_strcpy(Uint16 *dst, const Uint16 *src, int swap)
{
	if ( swap ) {
		while ( *src ) {
			*dst = SDL_Swap16(*src);
...
/* UTF32 string utilities */
static __inline__ int UTF32_strlen(const Uint32 *text)
{
	int size = 0;
	while ( *text++ ) {
		++size;
...
static __inline__ void UTF32_strcpy(Uint32 *dst, const Uint32 *src, int swap)
{
	if ( swap ) {
		while ( *src ) {
			*dst = SDL_Swap32(*src);
...
TTF_Font* TTF_OpenFontIndexRW( SDL_RWops *src, int freesrc, int ptsize, long index )
{
	...
	font->glyph_italics = 0.207f * font->height;
...
static Uint32 *UTF16_to_UTF32(Uint32 *UTF32, const Uint16 *text, int len)
{
	int i;

	for ( i=0; i < len; ++i ) {
		UTF32[i] = ((const Uint16 *)text)[i];
	}
	UTF32[i] = 0;
...
static Uint32 *UTF8_to_UTF32(Uint32 *UTF32, const char *utf8, int len)
{
	int i, j;
	Uint32 ch;
	...
	for ( i=0, j=0; i < len; ++i, ++j ) {
		...
		if ( ch >= 0xFC ) {
		    ch  =  (Uint32)(utf8[i]&0x01) << 30;
		    ch |=  (Uint32)(utf8[++i]&0x3F) << 24;
		    ch |=  (Uint32)(utf8[++i]&0x3F) << 18;
			ch |=  (Uint32)(utf8[++i]&0x3F) << 12;
			ch |=  (Uint32)(utf8[++i]&0x3F) << 6;
			ch |=  (Uint32)(utf8[++i]&0x3F);
		} else
		if ( ch >= 0xF8 ) {
		    ch  =  (Uint32)(utf8[i]&0x03) << 24;
		    ch |=  (Uint32)(utf8[++i]&0x3F) << 18;
			ch |=  (Uint32)(utf8[++i]&0x3F) << 12;
			ch |=  (Uint32)(utf8[++i]&0x3F) << 6;
			ch |=  (Uint32)(utf8[++i]&0x3F);
...
int TTF_SizeUTF8(TTF_Font *font, const char *text, int *w, int *h)
{
	...
	/* Copy the UTF-8 text to a UTF32 text buffer */
	UTF32_len = strlen(text);
	UTF32_text = (Uint32 *)ALLOCA((1+UTF32_len+1)*(sizeof *UTF32_text));
	if ( UTF32_text == NULL ) {
		TTF_SetError("Out of memory");
		return -1;
	}
	*UTF32_text = UNICODE_BOM_NATIVE;
	UTF8_to_UTF32(UTF32_text+1, text, UTF32_len);

	/* Render the new text */
	status = TTF_SizeUTF32(font, UTF32_text, w, h);
...
int TTF_SizeUTF16(TTF_Font *font, const Uint16 *text, int *w, int *h)
{
	Uint32 *UTF32_text;
	int UTF32_len;
	int status;

	/* Copy the UTF-16 text to a UTF32 text buffer */
	UTF32_len = UTF16_strlen(text);
	UTF32_text = (Uint32 *)ALLOCA((1+UTF32_len+1)*(sizeof *UTF32_text));
	if ( UTF32_text == NULL ) {
		TTF_SetError("Out of memory");
		return -1;
	...
	*UTF32_text = UNICODE_BOM_NATIVE;
	UTF16_to_UTF32(UTF32_text+1, text, UTF32_len);
...
int TTF_SizeUTF32(TTF_Font *font, const Uint32 *text, int *w, int *h)
{
	int status;
	const Uint32 *ch;
	int swapped;
...
/* Convert the UTF-8 text to UTF32 and render it
*/
				...
				const char *text, SDL_Color fg)
{
	...
	*UTF32_text = UNICODE_BOM_NATIVE;
	UTF8_to_UTF32(UTF32_text, text, UTF32_len);

	/* Render the new text */
	textbuf = TTF_RenderUTF32_Solid(font, UTF32_text, fg);
...
/* Convert the UTF-16 text to UTF32 and render it
*/
SDL_Surface *TTF_RenderUTF16_Solid(TTF_Font *font,
				const Uint16 *text, SDL_Color fg)
{
	...
	*UTF32_text = UNICODE_BOM_NATIVE;
	UTF16_to_UTF32(UTF32_text, text, UTF32_len);
...
SDL_Surface *TTF_RenderUTF32_Solid(TTF_Font *font,
				const Uint32 *text, SDL_Color fg)
{
	...
	if( ( TTF_SizeUTF32(font, text, &width, &height) < 0 ) || !width ) {
				...
				const char *text, SDL_Color fg, SDL_Color bg)
{
	...
	*UTF32_text = UNICODE_BOM_NATIVE;
	...
	/* Render the new text */
	textbuf = TTF_RenderUTF32_Shaded(font, UTF32_text, fg, bg);
...
/* Convert the UTF-16 text to UTF32 and render it
*/
SDL_Surface *TTF_RenderUTF16_Shaded(TTF_Font *font,
...
SDL_Surface* TTF_RenderUTF32_Shaded( TTF_Font* font,
                                     const Uint32* text,
				                     SDL_Color fg,
				                     SDL_Color bg )
{
				...
				const char *text, SDL_Color fg)
{
	...
	*UTF32_text = UNICODE_BOM_NATIVE;
	...
	/* Render the new text */
	textbuf = TTF_RenderUTF32_Blended(font, UTF32_text, fg);
...
/* Convert the UTF-16 text to UTF32 and render it
*/
SDL_Surface *TTF_RenderUTF16_Blended(TTF_Font *font,
				const Uint16 *text, SDL_Color fg)
{
	SDL_Surface *textbuf;
	Uint32 *UTF32_text;
...
SDL_Surface *TTF_RenderUTF32_Blended(TTF_Font *font,
				const Uint32 *text, SDL_Color fg)
{
	int xstart;
	int width, height;
...
void TTF_SetFontKegel( TTF_Font** font, int height )
{
    if( *font && height && (*font)->height != height ) {
        TTF_Font* new_font;
        SDL_RWops *rw = (*font)->src;
        int style = (*font)->style;
        int outline = (*font)->outline;
		...
		new_font = TTF_OpenFontIndexRW(rw, 1, height, 0);
		*font = new_font;
        ...
        if( kerning != (*font)->kerning ) {
            TTF_SetFontKerning(*font, kerning);
        }
        if( hinting != (*font)->hinting ) {
